...
def update(frame):
    ...
    # Center plot on Andromeda
    center_x, center_y = positions_r2[frame]
    width = 3e21
    ax.set_xlim(center_x - width / 1.5, center_x + width / 1.5)
    ax.set_ylim(center_y - width / 1.5, center_y + width / 1.5)

    return mw_path, and_path, mw_dot, and_dot

# Animation
ani = FuncAnimation(fig, update, frames=len(positions_r2),
