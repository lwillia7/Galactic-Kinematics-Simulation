...
# Time settings
t_max = 1e17  # Increased total time for a more noticeable effect (3.2 billion years)
dt = 5e12     # Time step (160000 years) good balance for accuracy and computational time

# Dynamical Friction parameters
rho = 1e-2  # Effective density parameter 
v0 = np.linalg.norm(v2)
ln_lambda = np.log(1e6)  # Logarithmic Coulomb factor which is part of the formula

# Dynamical friction calculation
def dynamical_friction(v2, r2, m2, rho, v0, ln_lambda):
    # Formula for dynamical friction 
    v_rel = np.linalg.norm(v2)
    return - (4 * np.pi * G * rho * m2 * v_rel) / (v_rel ** 3) * ln_lambda

# RK4 Method with Dynamical Friction
def rk4_step(r1, v1, r2, v2, dt):
    ...
    def derivatives(r1, v1, r2, v2):
        ...
        # Apply dynamical friction force to Andromedas acceleration
        friction = dynamical_friction(v2, r2, m2, rho, v0, ln_lambda)
        a2 += friction / m2  # Adjustment of acceleration of Andromeda due to dynamical friction
...
velocities_r2 = [np.linalg.norm(v2)]  
t = 0.0
...
while t < t_max:
    ...
    velocities_r2.append(np.linalg.norm(v2))  
    t += dt

positions_r1 = np.array(positions_r1)
positions_r2 = np.array(positions_r2)

# Andromedas velocity over time
plt.figure(figsize=(8, 6))
plt.plot(np.arange(len(velocities_r2)) * dt, velocities_r2)
plt.xlabel("Time (s)")
plt.ylabel("Velocity of Andromeda (m/s)")
plt.title("Velocity of Andromeda Over Time with Dynamical Friction")
plt.grid(True)
plt.show()
...
ax.set_title("Milky Way–Andromeda Collision with Dynamical Friction")
ax.grid(True)
ax.set_aspect('equal')
...
def update(frame):
    ...
    # Center between the galaxies
    center_x, center_y = positions_r1[frame]
    center_x2, center_y2 = positions_r2[frame]
    center_x3 = (center_x+center_x2)/2
    center_y3 = (center_y+center_y2)/2

    width = 3e21
    ax.set_xlim(center_x3 - width / 1, center_x3 + width / 1)
    ax.set_ylim(center_y3 - width / 1, center_y3 + width / 1)
